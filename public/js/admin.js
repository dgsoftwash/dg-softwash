...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.toggleWorkOrderStatus = async function(woId, field, newVal) {
    try {
      ...
      if (data.email_sent === 'job_complete') showPricingMsg('Service complete & payment reminder emailed to customer!', true);
      if (data.email_sent === 'invoice') showPricingMsg('Invoice emailed to customer (30 day terms)!', true);
      if (data.email_sent === 'paid') showPricingMsg('Payment receipt emailed to customer!', true);
      if (data.email_sent === 'status_paid') showPricingMsg('Payment received confirmation emailed to customer!', true);
    } catch (err) {
    ...
    fetch('/api/admin/year-end-report?year=' + year, { headers: { 'x-admin-token': adminToken } })
      ...
      .then(function(d) {
        ...
        var html = '<!DOCTYPE html><html><head><meta charset="utf-8">' +
          ...
          '</style></head><body>' +
          '<div class="no-print"><button class="print-btn" onclick="window.print()"><button class="print-btn" onclick="window.print()">&#128438; Print / Save as PDF</button></div>#128438; Print / Save as PDF</button> <button class="print-btn" onclick="window.close();window.history.back()" style="margin-left:10px;">← Back</button></div>' +
          ...
          '<div class="no-print" style="margin-top:16px;"><button class="print-btn" onclick="window.print()"><button class="print-btn" onclick="window.print()">&#128438; Print / Save as PDF</button></div>#128438; Print / Save as PDF</button> <button class="print-btn" onclick="window.close();window.history.back()" style="margin-left:10px;">← Back</button></div>' +
          '</body></html>';
