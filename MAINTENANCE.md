...
| iCloud backup alert from monitor | Backup stale >25h or 2TB HDD unmounted | Check `sudo launchctl list | grep icloud` — daemon must be loaded; check `cat /tmp/icloud-backup.log` |
| Backblaze alert from monitor | License expired/deleted or service stopped | Check `/Library/Backblaze.bzpkg/bzdata/overviewstatus.xml`; restart: `sudo launchctl kickstart -k system/com.backblaze.bzserv` |
| PostgreSQL alert from monitor | pg_isready failing | Run `pg_isready -U david -d dgsoftwash`; if fails: `brew services restart postgresql@15` |
...
**What it shows:**
- Services: Website (HTTPS + latency), Node App (PM2 status/restarts/uptime), PostgreSQL (pg_isready + query latency), Cloudflare Tunnel, UPS, Backblaze (XML status), iCloud backup (filesystem freshness), DB backup schedule (every 6h)
...
- DB dumps: `/Volumes/2TB HDD/Backups/database/` (keeps last 7) — runs every 6 hours (3 AM, 9 AM, 3 PM, 9 PM) via `com.dgsoftwash.database-backup.plist`
- Log: `/Volumes/2TB HDD/backup_log.txt` (script internal log) + `/tmp/icloud-backup.log` (launchd stdout/stderr)
- DB backup log: `/tmp/database-backup.log`

> **Dependency: nodemailer (global)**  
> The database backup email step requires nodemailer installed globally on the Mac:  
> `npm install -g nodemailer`  
> If backup emails stop working after a fresh setup, run this command first. The script uses `require('nodemailer')` (global, not from project node_modules).
...
### Monitor Script Enhancements (2026-04-25)

Four changes applied to the monitoring and backup configuration:

**1. DB backup every 6 hours**
- `com.dgsoftwash.database-backup.plist` updated from daily 3 AM to 4× daily:
  3 AM, 9 AM, 3 PM, 9 PM using `StartCalendarInterval` array
- Most recent dump is now never more than 6 hours old
- Check: `cat /tmp/database-backup.log | tail -20`

**2. PostgreSQL health check in dg-monitor.sh**
- Added `pg_isready -U david -d dgsoftwash` check in section 2 (after PM2 check)
- Fires macOS notification + Zoho email if pg_isready returns non-zero exit code
- This is a lower-level check than the existing DB query check — catches cases
  where PostgreSQL is up but rejecting connections before the server can query it

**3. iCloud backup freshness check in dg-monitor.sh**
- Added `find` check on `/Volumes/2TB HDD/Backups/iCloud Drive` in section 5
- Alerts if nothing in that directory was modified in the last 25 hours (1500 min)
- Fires macOS notification + Zoho email if stale or destination is missing
- Also alerts if the 2TB HDD appears unmounted (directory not found)

**4. Backblaze status check in dg-monitor.sh**
- Added XML-based check in section 6 reading:
  - `/Library/Backblaze.bzpkg/bzdata/bzreports/bzdc_synchostinfo.xml` — license status
  - `/Library/Backblaze.bzpkg/bzdata/overviewstatus.xml` — transmit state
- Alerts if XML files are missing (service down), license expired/deleted,
  or `cur_state` shows `error` or `stopped`

**Known issue — iCloud backup alert fires falsely:**
If the iCloud backup alert fires but you believe the backup ran, check that
the root LaunchDaemon is loaded:
```bash
sudo launchctl list | grep icloud
```
It should appear with exit code 0. If missing, the daemon needs to be reloaded:
```bash
sudo launchctl load /Library/LaunchDaemons/com.dgsoftwash.icloud-backup.plist
```
Also check the backup log for errors: `cat /tmp/icloud-backup.log | tail -30`
...
## CHANGES — April 10, 2026

### Security Hardening

- **Zoho SMTP credentials moved to `.env`** — `ZOHO_USER` and `ZOHO_APP_PASSWORD` env vars (was hardcoded)
- **Gabe admin password moved to `.env`** — `GABE_ADMIN_PASSWORD` env var (was hardcoded)
- **Admin tokens persisted to `.admin-tokens.json`** — tokens now survive PM2 restarts/reloads; previously, every server restart invalidated all admin sessions
- **bcrypt for admin password comparison** — `bcrypt.compare()` replaces plain string equality check on login
- **Rate limiting added** to public write endpoints via `express-rate-limit` (10 req / 15 min per IP):
  - `POST /api/contact`
  - `POST /api/reviews`
...
- **Booking confirmation screen** — on successful form submit, the contact form is replaced with a confirmation panel showing:
  - Date, time, service, price, and reference number (format: `BK-{id}`)
  - "We'll confirm within 24 hours" message
- **Pricing calculator → contact form URL params** — calculator passes estimate data via query string instead of sessionStorage; works correctly in private/incognito browsing

### Calendar

- **`GET /api/calendar-config` endpoint** — time slots and service durations are now served dynamically from the server instead of being hardcoded in `calendar.js`; calendar reads this endpoint on load

### Admin Panel

- **"+ New Booking" button** added to the Dashboard tab header for quick booking entry
- **Work order photo uploads** — photos can be attached to any work order:
  - `POST /api/admin/work-orders/:id/photos` — upload photo (stored in `work_order_photos` table)
  - `GET /api/admin/work-orders/:id/photos` — list photos for a work order
  - `DELETE /api/admin/work-orders/:id/photos/:photoId` — delete a photo
- **PDF invoice generation** — `GET /api/admin/work-orders/:id/invoice` generates a PDF invoice via `pdfkit`; returns `application/pdf`
- **Recurring expenses auto-post daily** — `processRecurringExpenses()` runs at midnight via cron; auto-creates expense records for recurring entries
- **Recurring services auto-schedule bookings daily** — `processRecurringServices()` runs at midnight; creates future bookings for recurring service contracts
- **Admin panel mobile-responsive** — `@media (max-width: 768px)` rules added to `styles.css`

### Backup

- **`backup.sh` cleanup fix** — added `rm -f /tmp/backup-status.json` at the top of the script so the dashboard backup status tile always reflects the current run (was sometimes showing stale status from a previous run)
- Verified: nightly DB backup, photos backup, and weekly code backup all operational

### Server Stability

- **PM2 yellow threshold raised 50 → 100** restarts (reduces false-alarm yellow status in monitoring)
- **Root cause fixed for 50 restarts** — 5 AM test alarm script was pointing to `/tmp/test-runner-fixed.js` which had been deleted; removed the dangling reference
- **`test-runner.js` security fix** — `dotenv` added, all credentials (SMTP, admin password) now read from env vars instead of being hardcoded

---

## 2026-04-13/14 — Monitoring Improvements & Cleanup

### server-alert.js: Power Outage Detection
- Added immediate email to `service@dgsoftwash.com` (→ text) when Mac Mini switches from AC to UPS battery — was previously only alerting at ≤20% battery remaining
- Added "All Clear" email when AC power is restored after a battery event
- Power state persisted to `/tmp/server-alert-power-state.txt` between 5-minute poll runs — ensures a single alert fires per AC→battery or battery→AC transition, not on every poll
- Zoho SMTP password moved from hardcoded plaintext to `.env` (`ZOHO_APP_PASSWORD`) — already completed on 2026-04-12; confirmed in place
- `dotenv` path fixed to use absolute require path (`/Volumes/1TB SSD/dg-softwash/node_modules/dotenv`) so LaunchAgent environment (no shell PATH) can always find the module

### dg-monitor.sh: PATH Fix
- Added `/opt/homebrew/bin` to PATH at the top of the script — `pm2` was not found in the non-login-shell LaunchAgent context, causing false-positive restart attempts every hour

### Cowork Scheduled Monitor
- `dg-softwash-monitor` scheduled agent (Cowork/OpenClaw): **disabled** — replaced by the native Mac LaunchAgent (`com.dgsoftwash.server-alert`) which runs unattended without requiring the OpenClaw gateway

### LaunchAgent Cleanup
- `com.dgsoftwash.testing` LaunchAgent: **removed** — was pointing to the deleted `/tmp/test-runner-fixed.js`, causing PM2 to accumulate restarts every time launchd fired it

---

## 2026-04-12 — PM2 Crash Fix / Server Stability

### PM2 Crash: yahoo-to-zoho-sync.py spawn failing
- **Root cause:** `runEmailSync()` in `server.js` spawned `/opt/homebrew/bin/python3` which does not exist on this machine. The `spawn` call had no `'error'` event handler, so the `ENOENT` became an unhandled exception that crashed PM2 (restarted 5 times).
- **Fix:** Updated spawn path to `/usr/bin/python3` (the correct system Python 3.9.6 location). Added `.on('error', ...)` handler to `runEmailSync()` so any future spawn failures log a warning and are non-fatal — the server will not crash.
- **Script status:** `/Volumes/1TB SSD/openclaw/workspace/scripts/yahoo-to-zoho-sync.py` exists and is present.
- **PM2 restarted** and confirmed stable (same PID, no new restarts after fix).

### server-alert.js: Zoho password moved to .env
- `ZOHO_APP_PASSWORD` is now read from `.env` — was previously hardcoded. No code change needed today; already in place.

### dg-monitor.sh PATH fix
- Added `/opt/homebrew/bin` to PATH in the LaunchAgent shell so `pm2` resolves correctly in the non-login-shell LaunchAgent context (was getting `pm2: command not found`).

---

## 2026-04-13 — Monitoring Improvements & Cleanup

### Power Outage Alerting (server-alert.js)
- Added immediate email alert to service@dgsoftwash.com when Mac Mini switches from AC to UPS battery (previously only alerted at ≤20% battery)
- Added "All Clear" email when AC power is restored
- Power state persisted between runs in /tmp/server-alert-power-state.txt so alert fires once per event, not every 5-minute poll
- Alert format: "⚡ Power Outage — Mac Mini on UPS battery at X%" and "✅ Power Restored — back on AC"

### Security (server-alert.js)
- Zoho SMTP password removed from hardcoded plaintext; now loaded from .env via ZOHO_APP_PASSWORD
- dotenv require uses absolute path to project node_modules so LaunchAgent can find it

### Monitor Stability (dg-monitor.sh)
- Fixed PATH issue: added /opt/homebrew/bin so pm2 resolves node correctly in LaunchAgent context
- Was causing false-positive restarts on every hourly run

### Cleanup
- com.dgsoftwash.testing LaunchAgent removed — was pointing to deleted /tmp/test-runner-fixed.js
- Cowork scheduled monitor (dg-softwash-monitor) disabled — replaced by native Mac LaunchAgent
